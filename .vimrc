...
augroup c_like_filetypes
  autocmd!
  autocmd FileType c,javascript nnoremap <F4> :execute '!cp -R $HOME/.vim/templates/js/. .' \| !npm install eslint prettier --save-dev<CR>
  autocmd FileType c,javascript nnoremap <buffer> gz :silent! normal! k<CR>]/V[/:silent! s/\zs\/\=\*\{1,2} *\ze\S\{2,}\\|\S\+\zs *\*\/\ze/<CR>`<V`>:silent! g/\/\*\{1,2}\\|\*\//d<CR>`<^:nohlsearch<CR>
  autocmd FileType c,javascript nnoremap <buffer> <Leader>? A<Space>?<CR>
  autocmd FileType c,javascript nnoremap <buffer> <Leader><CR> o;<Left>
  autocmd FileType c,javascript nnoremap <buffer> <Leader>, A;<Esc>
  autocmd FileType c,javascript nnoremap <buffer> <Leader>. A {<C-O>o}<C-O>O
  autocmd FileType c,javascript nnoremap <buffer> <Leader>/ g_i {<CR>}<C-O>O
  autocmd FileType c,javascript inoremap <buffer> $ ${}<Left>
  autocmd FileType c,javascript inoremap <buffer> & &&
  autocmd FileType c,javascript inoremap <buffer> <BAR> <BAR><BAR>
  autocmd FileType c,javascript inoremap <buffer> <Leader><CR> <C-O>o;<Left>
  autocmd FileType c,javascript inoremap <buffer> <Leader>, <Esc>^yg_S;<Left><C-R>0
  autocmd FileType c,javascript inoremap <buffer> <Leader>. <C-O>A {<C-O>o}<C-O>O
  autocmd FileType c,javascript inoremap <buffer> <Leader>/ <C-O>g_ {<CR>}<C-O>O
  autocmd FileType c,javascript inoremap <buffer> <Leader>? <Space>?<Space>
  autocmd FileType c,javascript inoremap <buffer> <Leader>: <Space>:<Space>
  autocmd FileType c,javascript inoremap <buffer> <Leader>= <Space>=<Space>
  autocmd FileType c,javascript inoremap <buffer> <Leader>* <Space>*<Space>
  autocmd FileType c,javascript inoremap <buffer> <Leader>- <Space>-<Space>
  ...
  autocmd FileType c,javascript inoremap <buffer> <Leader>> <Space>><Space>
  autocmd FileType c,javascript inoremap <buffer> <Leader>< <Space><<Space>
  autocmd FileType c,javascript vnoremap <buffer> gb c/*<CR>/<C-D><Esc>P
  autocmd FileType c,javascript vnoremap <buffer> gx <Esc>`<^i/* <C-O>`> */<Esc>
  autocmd FileType c,javascript vnoremap <buffer> gz :silent! s/\zs\/\=\*\{1,2} *\ze\S\{2,}\\|\S\+\zs *\*\/\ze/<CR>`<V`>:silent! g/\/\*\{1,2}\\|\*\//d<CR>`<^:nohlsearch<CR>
augroup END
" }}}
